...
import { copying } from "@/api/file_management.js";
...
export default {
  ...
  data() {
    return {
      ...
      tabsarry: [
        { name: "发文抄送", value: 1, icon: "分发发文" },
        { name: "收文抄送", value: 2, icon: "分发收文" },
        { name: "签报抄送", value: 3, icon: "分发收文" }
      ],
      columns_list: [
        [
          // 表单格式
  ...
  methods: {
    ...
    //获取归档箱数据
    async get_copying_list(
      page = 1,
      val = "",
      table_data = { order_field: null, sort: null },
      listRow
    ) {
      let { data } = await copying({
        status: this.file_type,
        page,
        keyword: val,
        type: 1,
        ...
        circulation_time: this.obj.one["抄送时间"]?.arry.join(","),
        issue_time: this.obj.one["签发时间"]?.arry.join(","),
        draft_time: this.obj.one["拟稿时间"]?.arry.join(",")
      });
    ...
    page_change(page, val, table_data, listRow) {
      this.get_copying_list(page, val, table_data, listRow);
    },
    //排序改变重新获取数据
    ...
    documentarea_search(val) {
      this.get_copying_list(1, val);
    },
    //数据条数改变
    page_size_change(val, table_data, listRow) {
      this.get_copying_list(1, val, table_data, listRow);
    },
    // tabs选项卡切换事件
    changeTab(e) {
      this.file_type = e;
    ...
    // 行点击事件
    rowClick(record) {
      this.$router.push({ name: "seefile", params: { id: 1 } });
    }
  }
};
</script>
