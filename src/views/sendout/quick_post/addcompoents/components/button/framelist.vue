<template>
  <div class="list-box">
    <div class="list-box-cl list-box-left">
      <div>
        <a-input-search placeholder="请输入姓名部门名称" v-model="kwd" enter-button @search="onSearch" />
      </div>
      <div>
        <a-tabs default-active-key="1" @change="callback">
          ...
          <a-tab-pane key="3" v-if="tag.xtjs" tab="系统角色">
            <Tree :tabs="treeRole" :res="tag" @treeClick="treeClick" />
          </a-tab-pane>
          <a-tab-pane key="4" v-if="tag.zjsy" tab="最近选择">
            <Tree :tabs="treeChoices" :res="tag" @treeClick="treeClick" />
          </a-tab-pane>
        </a-tabs>
      </div>
    </div>
      ...
      <div class="list-box-right-bottom">
        <template v-for="item in range.visual">
          <div class="list-box-right-bottom-cl" :key="item.value">
            <div>
              <a-icon
                :type="item.type==0?'user':item.type==1?'folder':item.type==5?'usergroup-delete':'tag'"
...
export default {
  ...
  props: ["range", "tag", "restype","visible"],
  components: {
    Tree
  },
  mounted() {
