      ...
      <div class="addformwork">
        <a-tabs default-active-key="1" @change="nums">
          ...
          <a-tab-pane key="3">
            ...
            <CenterText @onChangeMain="onChangeMain" />
          </a-tab-pane>
          <a-tab-pane key="4">
            <span slot="tab">
...
export default {
  ...
  created() {
    this.oncustem();
    // 接收到正文内容
    this.$bus.$on("postsetcentertextAdd", (val) => {
  ...
  methods: {
    ...
    // 保存按钮
    keep() {
      this.$bus.$emit("postsetcentertext");
    ...
    onHandleOk() {
      this.$router.push({
        path: "/draft/addDraft",
        query: { type: 1, id: this.templateId },
      });
    },
    ...
    // 传来的字段信息
    oncustem() {
      this.$bus.$on("setworkdocument", (val) => {
