<template>
  <div class="header flex-col">
    ...
    <a-modal v-model="visible" cancelText="取消" okText="确定" title="新建发文分类" @ok="handleOk">
...
export default {
  ...
  methods: {
    ...
    onAddDocumentExcelout() {
      this.$bus.$emit("postclassifysetExcelout");
    },
    onSearch() {
      this.defaultCurrent = 1;
      this.$bus.$emit("postclasspage", 1);
      this.getpost();
    },
    showModal() {
      this.visible = true;
    ...
    getpost() {
      ...
      if (this.keyword == "") {
        Getpost({
          ...
          if (res.code == 1) {
            this.$bus.$emit("postclassifysetGetpostList", {
              data: res.data,
              getpostList: false,
              total: res.count
            });
    ...
    addpost() {
      // 点击确定发送新建发文分类请求
      Addpost({ name: this.form.name, type: 1 }).then(res => {
        if (res.code == 1) {
          // 通知重新渲染页面
          this.$bus.$emit("postclassifysetReloadList");
        }
      });
    }
  }
