...
export default {
  ...
  created() {
    ...
    this.getdataList();
    // 接到新建通知 重新渲染页面
    this.$bus.$on("postclassifysetReloadList", val => {
      this.getdataList();
    });
    // 发送页数和显示条数给其他组件
    this.$bus.$emit("postclassifysetSendpage", {
  ...
  mounted() {
    ...
    this.$bus.$off("postclassifysetExcelout"); //先解绑，不然会多次出发
    // 接收点击导出事件
    this.$bus.$on("postclassifysetExcelout", () => {
  ...
  methods: {
    ...
    setpost() {
      // 点击确定按钮发送ajax请求
      Setpost({ name: this.form.name, id: this.row.id, type: 1 }).then(res => {
        // 修改成功之后更新列表
        if (res.code == 1) {
          this.getdataList();
        }
    ...
    pagechangeifyset(val) {
      ...
      if (this.getpostList) {
        this.onList();
        this.$bus.$emit("postclassifysetSetpostList", {
          current: val,
    ...
    // 接收到查询以后的列表数据
    onList() {
      this.$bus.$on("postclassifysetGetpostList", val => {
        this.data = val.data;
    ...
    ChangeSort(p, f, s) {
      if (s.order) {
        ...
        this.$bus.$emit("postifyset", { sort_field: s.columnKey, sort: order });
        if (this.getpostList) {
          this.onList();
        } else {
          this.getdataList();
