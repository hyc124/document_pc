<template>
  <div>
    ...
    <Pagetion
      @pageChange="pagechangepostwork"
      @currentchange="currentchangepostwork"
      :paging="paging"
      class="Page"
    />
...
export default {
  ...
  created() {
    ...
    this.getdataList(),
      ...
      this.onList();
    // 发送页数和显示条数给其他组件
    this.$bus.$emit("signoffformworksetSendpage", {
      defaultCurrent: this.paging.current,
      defaultPageSize: this.paging.pageSize,
    });
    // 查询同时改变页码
    this.$bus.$on("signoffformworksetPage", (val) => {
  ...
  methods: {
    ...
    // 页码改变
    pagechangepostwork(val) {
      this.paging.current = val;
      if (this.getpostList) {
        this.onList();
        // 让查询的数据也改变
        this.$bus.$emit("signoffformworksetSetpostList", {
    ...
    // 查询列表
    onList() {
      this.$bus.$on("signoffformworksetsetGetpostList", (val) => {
    ...
    ChangeSort(p, f, s) {
      if (s.order) {
        ...
        this.$bus.$emit("signoffformworkset", {
