      ...
      <div class="addformwork">
        <a-tabs default-active-key="1" @change="nums">
          <a-tab-pane key="1">
            ...
            <BasicInfo
              @signoffformworksetname="onName"
              @signoffformworksetonGenre="onGenre"
              @signoffformworksetonRule="onRule"
              @signoffformworksetonProcedure="onProcedure"
              @signoffformworksetonCrimson="onCrimson"
              @signoffformworksetonCrimsonName="onCrimsonName"
            />
          </a-tab-pane>
          <a-tab-pane key="2">
            <span slot="tab">
              <SvgIcon item="分发发文" />
              <span class="addformwork-title">签报信息</span>
            </span>
          ...
          <a-tab-pane key="3">
            ...
            <CenterText @signoffformworksetonChangeMain="onChangeMain" />
          </a-tab-pane>
          <a-tab-pane key="4">
            <span slot="tab">
...
import BasicInfo from "./addcomponents/basicinfo.vue";
import PostInfo from "@/views/applicationset/components/templateset/postinfo.vue";
import CenterText from "./addcomponents/centertext.vue";
import Enclosure from "./addcomponents/enclosure.vue";
...
export default {
  ...
  data() {
    return {
      ...
      PostInfo: {
        type: 3,
        bus: "signoffformworksetsetworkdocument"
      },
      visible: false,
  ...
  created() {
    this.oncustem();
    // 接收正文内容
    this.$bus.$on("signoffformworksetcententvalue", val => {
  ...
  methods: {
    ...
    keep() {
      ...
      this.$bus.$emit("signoffformworksetworksetgetdocument");
    ...
    onHandleOk() {
      this.$router.push({
        path: "/draft/addDraft",
        query: { type: 3, id: this.templateId }
      });
    },
    ...
    // 传来的字段信息
    oncustem() {
      this.$bus.$on("signoffformworksetsetworkdocument", val => {
