...
export default {
  ...
  methods: {
    showModal() {
      this.$router.push({ name: "addgetformworkset" });
    },
    onSearch() {
      this.defaultCurrent = 1;
      this.$bus.$emit("getformworksetpage", 1);
      this.getpost();
    },
    // 查询收文列表
    getpost() {
      ...
      GetpostinfoList({
        ...
        if (res.code == 1) {
          this.$bus.$emit("getformworksetGetpostList", {
            data: res.data,
            getpostList: true,
            total: res.count
          });
