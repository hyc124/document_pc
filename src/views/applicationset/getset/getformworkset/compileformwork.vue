      ...
      <div class="addformwork">
        <a-tabs default-active-key="1" @change="nums">
          <a-tab-pane key="1">
            ...
            <BasicInfo
              @compilegetformname="onName"
              @compilegetformonGenre="onGenre"
              @compilegetformonRule="onRule"
              @compilegetformonProcedure="onProcedure"
              @compilegetformonCrimson="onCrimson"
              @compilegetformonCrimsonName="onCrimsonName"
              :Listdata="Listdata"
            />
          </a-tab-pane>
          <a-tab-pane key="2">
...
export default {
  ...
  data() {
    return {
      ...
      PostInfo: {
        bus: "compilegetformsetworkdocument",
        type:2
      }
    };
  },
  ...
  methods: {
    ...
    keep() {
      if (this.setInfo) {
        this.$bus.$emit("compilegetformworksetgetdocument");
    ...
    // 传来的字段信息
    oncustem() {
      this.$bus.$on("compilegetformsetworkdocument", val => {
    ...
    oncentertext() {
      this.$bus.$on("compilegetformcentertext", val => {
        this.data.content_type = val;
      });
    },
    // 获取公文模板详情
