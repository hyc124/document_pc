      ...
      <div class="head">
        <div class="left">
          <a-input-search
            ...
            :maxLength="20"
          />
          <span @click="change_search" class="open-search">
            {{SearchTriangle?"高级搜索":"收起搜索"}}
            <i class="triangle-top" v-if="SearchTriangle"></i>
            <i class="triangle" v-else></i>
          </span>
        </div>
        <div class="right">
          ...
          <a-button v-if="powers('post_print')" @click="batchPrinting(1)" class="btn">批量打印</a-button>
        </div>
      </div>
      <SeniorSearch class="SeniorSearch" :obj="obj" @oninSearch="oninSearch" ref="searchDetail" />
      <a-tabs type="card" v-model="status" @change="onChange">
        ...
        <a-tab-pane :key="5" tab="草稿"></a-tab-pane>
      </a-tabs>
      <!-- 列表内容 -->
      <div class="table-content-at" :style="{height:`${height + 24 + 54}px`}">
        <a-table
          :columns="columns"
          :data-source="data"
          @change="Change"
        ...
        >
          ...
          <span slot="status" slot-scope="item,record">
            <span id="status" :style="{color:style_color[record.status]}">
...
export default {
  ...
  mounted() {
    this.height = Calculatedaltitude(60, 30, 20, 50, 67);
  },
  methods: {
    // 删除草稿
    editclick(info) {
    ...
    addlist() {
      ...
      libraryList({
        ...
        advanced: {
          ...
          copy_users: obj.one["收文单位"]?.intext, // 收文单位
          word_no: obj.one["收文字号"]?.intext, // 收文字号
            ...
            switch (record.r_status) {
              case 4:
                if (record.can_edit == 1) {
                  go_path = `/draft/addDraft?d_id=${record.id}&type=2&can_edit=true`;
                } else {
                  go_path = `/home/incoming?id=${record.id}&file_type=2`;
                }
                break;
              case 8:
                go_path = `/draft/addDraft?d_id=${record.id}&type=2`;
                break;
              default:
                go_path = `/home/incoming?id=${record.id}&file_type=2`;
            }
